 ...
 * `StandaloneAnalysisAPISession` registers exactly one source `KaModule` per project (see
 * `KotlinAnalysisAPISession`), and NetBeans itself has no multi-module Maven/Gradle reactor
 * awareness across separate open projects (see the multi-module architecture research done for
 * E9.7 Move Declaration: no `Module`/dependency graph exists anywhere in this codebase).
 *
 * [SingletonModule] represents "the one module" of a NetBeans [project] so IDEA's ported
 * conflict-detection code (module-dependency / internal-visibility / facet checks) can compile and
 * run against a real, correctly-behaving `Module`: identity comparisons (`moduleA == moduleB`)
 * always succeed since every element in the project resolves to the same instance, which is the
 * true state of affairs for every project type this plugin supports (Maven/Gradle/Ant, always
 * single-module). This is not a simplification of real behaviour — it is the correct model.
 */
