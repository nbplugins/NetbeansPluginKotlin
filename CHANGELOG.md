- Added **Inline Variable** refactoring (Ctrl+Alt+N / **Refactor → Inline Variable**) for Kotlin `val` properties: replaces every read reference with the property initializer text and deletes the declaration; the preview panel shows each occurrence as a separate checkbox so individual replacements can be skipped; undo rolls back all applied changes per occurrence.
