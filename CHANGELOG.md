- Added **Introduce Variable** refactoring (Ctrl+Alt+V / **Refactor → Introduce Variable**) for Kotlin expressions powered by the K2 Analysis API: extracts the expression under the caret (or selection) into a new `val` declaration inserted just above the enclosing statement, replaces all semantically equivalent occurrences in the same scope using `K2SemanticMatcher`, suggests a name from `KotlinNameSuggester`, and lets the user edit the name in an inline panel before applying; preview panel shows every occurrence; undo rolls the file back atomically. The **Refactor** menu is reordered to match IntelliJ IDEA's Kotlin menu (Rename → Move → Safely Delete → Extract Function → Introduce Variable → Inline → Undo/Redo).
