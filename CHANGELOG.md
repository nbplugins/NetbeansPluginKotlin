- Added Tools → Options → Kotlin top-level category with a Formatter → Other tab containing "Allow trailing comma in declarations" and "Allow trailing comma on call site" checkboxes; the Kotlin formatter picks up these settings on the next reformat. Tabs and Indents remain in Tools → Options → Editor → Formatting (Language: Kotlin) as before.
