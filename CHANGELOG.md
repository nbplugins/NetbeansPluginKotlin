- Fixed editor's vertical indent-guide bars and Tab key in `.kt` files ignoring Kotlin indent settings: the editor now reads `indent-shift-width`, `tab-size`, `spaces-per-tab`, and `expand-tabs` from the effective Kotlin formatter settings (per-project override if present, otherwise global from Tools → Options → Kotlin), and refreshes open documents immediately when settings are saved.
- Fixed: indent and tab settings from Tools → Options → Kotlin → Tabs & Indent are now respected when formatting; previously the formatter silently used the default 4 spaces and saved values were also lost across restarts.
- Removed redundant Kotlin entry under Tools → Options → Editor → Formatting; all Kotlin formatter settings now live under Tools → Options → Kotlin.
- Added per-project Kotlin formatter settings under Project Properties → Kotlin Formatter: the panel is identical to Tools → Options → Kotlin (same scheme bar, gear menu, 7 tabs, live preview), and a "Reset to IDE settings" button resets the panel to the global IDE settings; settings are saved lazily to `.idea/codeStyles/Project.xml` in IntelliJ IDEA's format — the file is created only when the project settings differ from the global IDE settings and deleted when they match again; Maven, J2SE/Ant, and Gradle projects are all supported.
- Added scheme management to Tools → Options → Kotlin: a gear button (⚙) next to the scheme selector opens a menu with Duplicate, Rename, Delete, Restore Defaults, Export…, and Import… actions; custom schemes are stored in NbPreferences and persist between sessions; Export/Import uses the standard IntelliJ IDEA `<code_scheme>` XML format so schemes are portable to/from IDEA.
- Added Kotlin formatter settings UI under Tools → Options → Kotlin: six tabs ("Tabs & Indent", "Spaces", "Wrapping & Braces", "Blank Lines", "Imports", "Other"), a scheme selector (Kotlin Coding Conventions / Kotlin Obsolete Coding Conventions / IDE Defaults), and a live preview pane that reformats a Kotlin code sample as settings are changed; the Spaces and Wrapping & Braces tabs are auto-generated from IDEA's Kotlin code-style provider and include all IDEA-equivalent options (including "Align when multiline" for binary operations and expression body functions). The Wrapping & Braces tab mirrors IDEA's structure: "Hard wrap at", "Wrap on typing", and "Visual guides" are the first three top-level rows; groups use "Function …" naming and IDEA's emission order ("Function declaration parameters" → "Function call arguments" → "Function parentheses" → "Chained function calls" → "'if()' statement" → "do...while() statement" → "try statement(s)" → …); wrap-mode combo selections (Don't wrap / Wrap if long / Wrap always) keep showing the label after a selection instead of repainting as a raw integer. The formatter picks up all these settings on the next reformat.
