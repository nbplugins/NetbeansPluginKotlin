- Added Kotlin formatter settings panels under Tools → Options → Kotlin: "Tabs & Indent" (tab size, indent size, continuation indent, use-tab-character), "Blank Lines" (around 'when' branches, before declaration with comment/annotation), and "Other" (allow trailing comma in declarations, allow trailing comma on call site); the formatter picks up all these settings on the next reformat.
