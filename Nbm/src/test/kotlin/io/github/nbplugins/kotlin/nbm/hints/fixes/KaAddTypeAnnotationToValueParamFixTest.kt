...
/**
 * Unit tests for [KaAddTypeAnnotationToValueParamFix].
 *
 * Verifies that [KaAddTypeAnnotationToValueParamFix.isApplicable] returns true for
 * VALUE_PARAMETER_WITHOUT_EXPLICIT_TYPE diagnostics and false for unrelated diagnostics.
 * Description: adds an explicit type annotation to a value parameter.
 */
class KaAddTypeAnnotationToValueParamFixTest : KotlinTestCase("KaAddTypeAnnotationToValueParamFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "VALUE_PARAMETER_WITHOUT_EXPLICIT_TYPE" } ?: return
        assertTrue(KaAddTypeAnnotationToValueParamFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "VALUE_PARAMETER_WITHOUT_EXPLICIT_TYPE" } ?: return
        assertFalse(KaAddTypeAnnotationToValueParamFix(error, doc, kaKtFile).isApplicable())
    }
}
