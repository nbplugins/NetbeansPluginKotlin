...
/**
 * Unit tests for [KaAddAccessorsFix].
 *
 * Verifies that [KaAddAccessorsFix.isApplicable] returns true for
 * MUST_BE_INITIALIZED diagnostics and false for unrelated diagnostics.
 * Description: adds getter/setter to a property that must be initialized.
 */
class KaAddAccessorsFixTest : KotlinTestCase("KaAddAccessorsFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "MUST_BE_INITIALIZED" } ?: return
        assertTrue(KaAddAccessorsFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "MUST_BE_INITIALIZED" } ?: return
        assertFalse(KaAddAccessorsFix(error, doc, kaKtFile).isApplicable())
    }
}
