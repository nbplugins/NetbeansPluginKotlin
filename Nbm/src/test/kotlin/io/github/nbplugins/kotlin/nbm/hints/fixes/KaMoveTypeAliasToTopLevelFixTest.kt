...
/**
 * Unit tests for [KaMoveTypeAliasToTopLevelFix].
 *
 * Verifies that [KaMoveTypeAliasToTopLevelFix.isApplicable] returns true for
 * TOPLEVEL_TYPEALIASES_ONLY diagnostics and false for unrelated diagnostics.
 * Description: moves a nested typealias to the top level.
 */
class KaMoveTypeAliasToTopLevelFixTest : KotlinTestCase("KaMoveTypeAliasToTopLevelFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "TOPLEVEL_TYPEALIASES_ONLY" } ?: return
        assertTrue(KaMoveTypeAliasToTopLevelFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "TOPLEVEL_TYPEALIASES_ONLY" } ?: return
        assertFalse(KaMoveTypeAliasToTopLevelFix(error, doc, kaKtFile).isApplicable())
    }
}
