...
/**
 * Unit tests for [KaAddInlineModifierFix].
 *
 * Verifies that [KaAddInlineModifierFix.isApplicable] returns true for
 * USAGE_IS_NOT_INLINABLE diagnostics and false for unrelated diagnostics.
 * Description: adds noinline to a lambda parameter.
 */
class KaAddInlineModifierFixTest : KotlinTestCase("KaAddInlineModifierFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "USAGE_IS_NOT_INLINABLE" } ?: return
        assertTrue(KaAddInlineModifierFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "USAGE_IS_NOT_INLINABLE" } ?: return
        assertFalse(KaAddInlineModifierFix(error, doc, kaKtFile).isApplicable())
    }
}
