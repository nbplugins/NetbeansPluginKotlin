...
/**
 * Unit tests for [KaAddValVarToConstructorParamFix].
 *
 * Verifies that [KaAddValVarToConstructorParamFix.isApplicable] returns true for
 * DATA_CLASS_NOT_PROPERTY_PARAMETER diagnostics and false for unrelated diagnostics.
 * Description: adds val to a data class constructor parameter missing val/var.
 */
class KaAddValVarToConstructorParamFixTest : KotlinTestCase("KaAddValVarToConstructorParamFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "DATA_CLASS_NOT_PROPERTY_PARAMETER" } ?: return
        assertTrue(KaAddValVarToConstructorParamFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "DATA_CLASS_NOT_PROPERTY_PARAMETER" } ?: return
        assertFalse(KaAddValVarToConstructorParamFix(error, doc, kaKtFile).isApplicable())
    }
}
