...
/**
 * Unit tests for [KaRemoveSupertypeFix].
 *
 * Verifies that [KaRemoveSupertypeFix.isApplicable] returns true for
 * MANY_CLASSES_IN_SUPERTYPE_LIST diagnostics and false for unrelated diagnostics.
 * Description: removes a redundant supertype entry.
 */
class KaRemoveSupertypeFixTest : KotlinTestCase("KaRemoveSupertypeFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "MANY_CLASSES_IN_SUPERTYPE_LIST" } ?: return
        assertTrue(KaRemoveSupertypeFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "MANY_CLASSES_IN_SUPERTYPE_LIST" } ?: return
        assertFalse(KaRemoveSupertypeFix(error, doc, kaKtFile).isApplicable())
    }
}
