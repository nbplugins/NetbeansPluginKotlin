...
/**
 * Unit tests for [KaAddSuspendModifierFix].
 *
 * Verifies that [KaAddSuspendModifierFix.isApplicable] returns true for
 * ILLEGAL_SUSPEND_FUNCTION_CALL diagnostics and false for unrelated diagnostics.
 * Description: adds the suspend modifier to the enclosing function.
 */
class KaAddSuspendModifierFixTest : KotlinTestCase("KaAddSuspendModifierFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "ILLEGAL_SUSPEND_FUNCTION_CALL" } ?: return
        assertTrue(KaAddSuspendModifierFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "ILLEGAL_SUSPEND_FUNCTION_CALL" } ?: return
        assertFalse(KaAddSuspendModifierFix(error, doc, kaKtFile).isApplicable())
    }
}
