...
/**
 * Unit tests for [KaRemoveArgumentFix].
 *
 * Verifies that [KaRemoveArgumentFix.isApplicable] returns true for
 * TOO_MANY_ARGUMENTS diagnostics and false for unrelated diagnostics.
 */
class KaRemoveArgumentFixTest : KotlinTestCase("KaRemoveArgumentFix", "quickfixes") {
    ...
    /** isApplicable returns true for TOO_MANY_ARGUMENTS. */
    fun testIsApplicableForTooManyArguments() {
        ...
        val error = errors.firstOrNull { it.key == "TOO_MANY_ARGUMENTS" } ?: return
        assertTrue(KaRemoveArgumentFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val doc = getDocumentForFileObject(file) ?: return
        val errors = getErrors(kaKtFile)
        // Use any non-TOO_MANY_ARGUMENTS error to verify false
        val error = errors.firstOrNull { it.key != "TOO_MANY_ARGUMENTS" } ?: return
        assertFalse(KaRemoveArgumentFix(error, doc, kaKtFile).isApplicable())
    }
}
