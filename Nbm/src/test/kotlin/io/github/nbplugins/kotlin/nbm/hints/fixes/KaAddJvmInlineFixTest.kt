...
/**
 * Unit tests for [KaAddJvmInlineFix].
 *
 * Verifies that [KaAddJvmInlineFix.isApplicable] returns true for
 * VALUE_CLASS_WITHOUT_JVM_INLINE_ANNOTATION diagnostics and false for unrelated diagnostics.
 * Description: adds @JvmInline to a value class missing the annotation.
 */
class KaAddJvmInlineFixTest : KotlinTestCase("KaAddJvmInlineFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "VALUE_CLASS_WITHOUT_JVM_INLINE_ANNOTATION" } ?: return
        assertTrue(KaAddJvmInlineFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "VALUE_CLASS_WITHOUT_JVM_INLINE_ANNOTATION" } ?: return
        assertFalse(KaAddJvmInlineFix(error, doc, kaKtFile).isApplicable())
    }
}
