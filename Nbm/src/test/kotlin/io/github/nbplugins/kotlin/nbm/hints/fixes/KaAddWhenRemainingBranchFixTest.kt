...
/**
 * Unit tests for [KaAddWhenRemainingBranchFix].
 *
 * Verifies that [KaAddWhenRemainingBranchFix.isApplicable] returns true for
 * NO_ELSE_IN_WHEN diagnostics and false for unrelated diagnostics.
 * Description: adds all missing branches to a non-exhaustive when expression.
 */
class KaAddWhenRemainingBranchFixTest : KotlinTestCase("KaAddWhenRemainingBranchFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "NO_ELSE_IN_WHEN" } ?: return
        assertTrue(KaAddWhenRemainingBranchFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "NO_ELSE_IN_WHEN" } ?: return
        assertFalse(KaAddWhenRemainingBranchFix(error, doc, kaKtFile).isApplicable())
    }
}
