...
/**
 * Unit tests for [KaChangeObjectToClassFix].
 *
 * Verifies that [KaChangeObjectToClassFix.isApplicable] returns true for
 * CONSTRUCTOR_IN_OBJECT diagnostics and false for unrelated diagnostics.
 * Description: converts an object with a constructor to a class.
 */
class KaChangeObjectToClassFixTest : KotlinTestCase("KaChangeObjectToClassFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "CONSTRUCTOR_IN_OBJECT" } ?: return
        assertTrue(KaChangeObjectToClassFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "CONSTRUCTOR_IN_OBJECT" } ?: return
        assertFalse(KaChangeObjectToClassFix(error, doc, kaKtFile).isApplicable())
    }
}
