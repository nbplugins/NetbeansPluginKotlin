...
/**
 * Unit tests for [KaRemoveDefaultParameterValueFix].
 *
 * Verifies that [KaRemoveDefaultParameterValueFix.isApplicable] returns true for
 * DEFAULT_VALUE_NOT_ALLOWED_IN_OVERRIDE diagnostics and false for unrelated diagnostics.
 * Description: removes a default parameter value from an overriding function.
 */
class KaRemoveDefaultParameterValueFixTest : KotlinTestCase("KaRemoveDefaultParameterValueFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "DEFAULT_VALUE_NOT_ALLOWED_IN_OVERRIDE" } ?: return
        assertTrue(KaRemoveDefaultParameterValueFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "DEFAULT_VALUE_NOT_ALLOWED_IN_OVERRIDE" } ?: return
        assertFalse(KaRemoveDefaultParameterValueFix(error, doc, kaKtFile).isApplicable())
    }
}
