...
/**
 * Unit tests for [KaAddReturnExpressionFix].
 *
 * Verifies that [KaAddReturnExpressionFix.isApplicable] returns true for
 * NO_RETURN_IN_FUNCTION_WITH_BLOCK_BODY diagnostics and false for unrelated diagnostics.
 * Description: adds a return expression to a block-body function missing a return statement.
 */
class KaAddReturnExpressionFixTest : KotlinTestCase("KaAddReturnExpressionFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "NO_RETURN_IN_FUNCTION_WITH_BLOCK_BODY" } ?: return
        assertTrue(KaAddReturnExpressionFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "NO_RETURN_IN_FUNCTION_WITH_BLOCK_BODY" } ?: return
        assertFalse(KaAddReturnExpressionFix(error, doc, kaKtFile).isApplicable())
    }
}
