...
/**
 * Unit tests for [KaTooLongCharLiteralFix].
 *
 * Verifies that [KaTooLongCharLiteralFix.isApplicable] returns true for
 * TOO_MANY_CHARACTERS_IN_CHARACTER_LITERAL diagnostics and false for unrelated diagnostics.
 * Description: converts a too-long character literal to a string.
 */
class KaTooLongCharLiteralFixTest : KotlinTestCase("KaTooLongCharLiteralFix", "quickfixes") {
    ...
    fun testIsApplicableForMatchingKey() {
        ...
        val error = errors.firstOrNull { it.key == "TOO_MANY_CHARACTERS_IN_CHARACTER_LITERAL" } ?: return
        assertTrue(KaTooLongCharLiteralFix(error, doc, kaKtFile).isApplicable())
    }
    ...
    fun testIsNotApplicableForOtherKey() {
        ...
        val errors = getErrors(kaKtFile)
        val error = errors.firstOrNull { it.key != "TOO_MANY_CHARACTERS_IN_CHARACTER_LITERAL" } ?: return
        assertFalse(KaTooLongCharLiteralFix(error, doc, kaKtFile).isApplicable())
    }
}
