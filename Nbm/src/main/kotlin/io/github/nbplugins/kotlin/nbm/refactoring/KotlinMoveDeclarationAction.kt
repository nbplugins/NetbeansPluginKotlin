/*******************************************************************************
 * Copyright 2000-2025 JetBrains s.r.o.
 * Copyright 2026 nbplugins contributors
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 *******************************************************************************/
package io.github.nbplugins.kotlin.nbm.refactoring

import io.github.nbplugins.kotlin.nbm.resolve.KotlinAnalysisAPISession
import io.github.nbplugins.kotlin.refactoring.KaMoveDeclarationComputer
import io.github.nbplugins.kotlin.refactoring.KaMoveDeclarationResult
import org.jetbrains.kotlin.log.KotlinLogger
import org.jetbrains.kotlin.utils.ProjectUtils
import org.netbeans.editor.BaseAction
import org.netbeans.modules.refactoring.spi.ui.UI
import org.openide.windows.TopComponent
import java.awt.event.ActionEvent
import javax.swing.text.JTextComponent
import javax.swing.text.StyledDocument

/**
 * Editor action for **Move Declaration** (F6) — moves the top-level declaration at the caret to a
 * different package/file, retargeting external references and detecting conflicts via IDEA's real
 * K2 move engine ([KaMoveDeclarationComputer]).
 *
 * Registered under action name [ACTION_NAME] in `layer.xml` for `text/x-kotlin` and exposed in the
 * Refactor menu.
 */
class KotlinMoveDeclarationAction : BaseAction(ACTION_NAME, SAVE_POSITION or ABBREV_RESET) {

    init {
        putValue(NAME, "Move Declaration...")
        putValue(SHORT_DESCRIPTION, "Move Declaration to Another File/Package")
        putValue(POPUP_MENU_TEXT, "Move Declaration...")
    }

    override fun actionPerformed(evt: ActionEvent, target: JTextComponent) {
        val doc = target.document as? StyledDocument ?: return
        val caretOffset = target.caretPosition

        runCatching {
            val result = resolveOutcome(doc, caretOffset) ?: return@runCatching
            val refactoring = KotlinMoveDeclarationRefactoring(doc, caretOffset)
            UI.openRefactoringUI(
                KotlinMoveDeclarationUI(result, refactoring),
                TopComponent.getRegistry().activated,
            )
        }.onFailure { e ->
            KotlinLogger.INSTANCE.logException("KotlinMoveDeclarationAction failed", e)
        }
    }

    /**
     * Runs a quick analysis pass and returns the initial result, or `null` when the caret is not
     * inside a top-level named declaration.
     */
    private fun resolveOutcome(doc: StyledDocument, caretOffset: Int): KaMoveDeclarationResult? {
        val fo = ProjectUtils.getFileObjectForDocument(doc) ?: return null
        val project = ProjectUtils.getKotlinProjectForFileObject(fo)
            ?: ProjectUtils.getValidProject()
            ?: return null
        return runCatching {
            val session = KotlinAnalysisAPISession.getSession(project)
            val ktFile = session.getKtFileForPath(fo.path) ?: return@runCatching null
            val computer = KaMoveDeclarationComputer(ktFile, caretOffset)
            when (val outcome = computer.compute()) {
                is KaMoveDeclarationComputer.Outcome.Ready -> outcome.result
                else -> null
            }
        }.getOrElse { null }
    }

    companion object {
        /** Action name used in layer.xml registration. */
        const val ACTION_NAME = "kotlin-move-declaration"
    }
}
