...
/**
 * Editor action for **Ctrl+Alt+P** — "Introduce Parameter" for Kotlin expressions inside a
 * function/secondary-constructor body (or a class body/property initializer, targeting its primary
 * constructor).
 *
 * Registered under action name [ACTION_NAME] in `layer.xml` for `text/x-kotlin`.
 *
 * On invocation, the action runs a quick analysis pass via [KaIntroduceParameterComputer] to check
 * applicability and collect scope candidates. If the expression is suitable, the refactoring UI
 * dialog is opened.
 */
class KotlinIntroduceParameterAction : BaseAction(ACTION_NAME, SAVE_POSITION or ABBREV_RESET) {

    init {
        putValue(NAME, "Introduce Parameter...")
        putValue(SHORT_DESCRIPTION, "Introduce Parameter")
        putValue(POPUP_MENU_TEXT, "Introduce Parameter...")
    }
    ...
    override fun actionPerformed(evt: ActionEvent, target: JTextComponent) {
        ...
        runCatching {
            ...
            val refactoring = KotlinIntroduceParameterRefactoring(doc, startOffset, endOffset)
            UI.openRefactoringUI(
                KotlinIntroduceParameterUI(result, scopeCandidates, refactoring),
                TopComponent.getRegistry().activated,
            )
        }.onFailure { e ->
            KotlinLogger.INSTANCE.logException("KotlinIntroduceParameterAction failed", e)
        }
    }

    /**
     * Runs a quick analysis pass and returns the initial result and scope candidates, or `null`
     * when the selection is not suitable for parameter introduction.
     *
     * @return pair of (KaIntroduceParameterResult, scopeCandidates), or `null` if not applicable
     */
    private fun resolveOutcome(
        doc: StyledDocument,
        startOffset: Int,
        endOffset: Int,
    ): Pair<KaIntroduceParameterResult, List<ScopeCandidate>>? {
        ...
        return runCatching {
            ...
            val computer = KaIntroduceParameterComputer(ktFile, startOffset, endOffset, session.session.project)
            val scopeCandidates = computer.collectScopeCandidates()
            when (val outcome = computer.compute()) {
                is KaIntroduceParameterComputer.Outcome.Ready -> outcome.result to scopeCandidates
                else -> null
            }
        }.getOrElse { null }
    }
    ...
    companion object {
        ...
        const val ACTION_NAME = "kotlin-introduce-parameter"
    }
}
