...
/**
 * Carrier [AbstractRefactoring] for the Kotlin **Introduce Variable** refactoring.
 *
 * This subclass exists so [KotlinRefactoringsFactory] can identify the refactoring kind via
 * `instanceof` and route it to [KotlinIntroduceVariablePlugin].
 *
 * [doc], [startOffset], and [endOffset] are exposed as plain fields (not via the Lookup) because
 * the Lookup cannot hold two `Int` values with distinct semantics.  After the UI dialog is
 * confirmed, [chosenName] holds the variable name the user entered.
 *
 ...
 */
class KotlinIntroduceVariableRefactoring(
