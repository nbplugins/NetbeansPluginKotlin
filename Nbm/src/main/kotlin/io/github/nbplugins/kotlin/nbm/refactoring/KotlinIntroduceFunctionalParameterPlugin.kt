 ...
 * call and returns a path→text map. Every touched NetBeans document is captured before the
 * hunk-only writes are staged through [KotlinRefactoringTransaction], so a failed participant
 * restores every earlier declaration/caller change exactly while unrelated text remains untouched.
 *
...
class KotlinIntroduceFunctionalParameterApplyElement(
    ...
    override fun performChange() {
        ...
        runCatching {
            ...
            val outcome = KaIntroduceFunctionalParameterComputer(
                callerKtFile, refactoring.startOffset, refactoring.endOffset, session.session.project,
            ).apply(request)
            if (outcome is KaIntroduceFunctionalParameterComputer.ApplyOutcome.Success) {
                ...
                outcome.fileTexts.forEach { (path, text) ->
                    val file = FileUtil.toFileObject(FileUtil.normalizeFile(java.io.File(path)))
                        ?: error("Introduce Functional Parameter could not resolve changed file $path.")
            ...
            } else if (outcome is KaIntroduceFunctionalParameterComputer.ApplyOutcome.Conflicts) {
                KotlinLogger.INSTANCE.logWarning("KotlinIntroduceFunctionalParameterApplyElement: change skipped, conflicts found: ${outcome.messages}")
            } else if (outcome is KaIntroduceFunctionalParameterComputer.ApplyOutcome.Error) {
                throw outcome.error
            }
        }.onFailure { KotlinLogger.INSTANCE.logException("KotlinIntroduceFunctionalParameterApplyElement.performChange failed", it) }
        runCatching { pendingTransaction?.rollback() }
            .onFailure { KotlinLogger.INSTANCE.logException("KotlinIntroduceFunctionalParameterApplyElement rollback failed", it) }
    ...
    override fun undoChange() {
        runCatching {
            ...
            transaction = null
            KotlinAnalysisAPISession.invalidate(nbProject)
        }.onFailure { KotlinLogger.INSTANCE.logException("KotlinIntroduceFunctionalParameterApplyElement.undoChange failed", it) }
    }
}
