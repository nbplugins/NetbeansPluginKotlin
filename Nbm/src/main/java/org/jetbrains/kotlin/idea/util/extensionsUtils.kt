...
fun <TCallable : CallableDescriptor> TCallable.substituteExtensionIfCallable(
    ...
    // Simplified: use only static receiver type (no smart cast variants in NetBeans)
    val types = receivers.map { it.type }
...
fun <TCallable : CallableDescriptor> TCallable.substituteExtensionIfCallable(
    ...
    // Simplified: use only static receiver type (no smart cast variants in NetBeans)
    return substituteExtensionIfCallable(listOf(receiver.type), callType)
}
