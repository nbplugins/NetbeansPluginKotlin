...
**Last commit:** `7702dd61` — Added Tabs & Indent and Blank Lines formatter panels; removed dead KotlinFormatterUI module  
**As of:** 2026-06-02
...
| Commit | Description |
|--------|-------------|
| *(pending)* | Added StyleBar, live preview pane; restructured KotlinOptionsPanel |
| `7702dd61` | Added Tabs & Indent and Blank Lines formatter panels; removed KotlinFormatterUI |
...
`Nbm/src/main/kotlin/io/github/nbplugins/kotlin/nbm/options/formatter/KotlinStyleBar.kt`

Style-preset selector shown at the top of the Kotlin Options panel. Dropdown with three entries:
- Kotlin Coding Conventions → applies `KotlinOfficialStyleGuide` (CODE_STYLE_ID = "KOTLIN_OFFICIAL")
- Kotlin Obsolete Coding Conventions → applies `KotlinObsoleteStyleGuide` (CODE_STYLE_ID = "KOTLIN_OLD_DEFAULTS")
- IDE Defaults → fresh `CodeStyleSettings()` with no preset

`setCurrentStyle(codeStyleId)` syncs the combo without firing `onStyleApplied`.

### KotlinFormattingPreviewPane

`Nbm/src/main/kotlin/io/github/nbplugins/kotlin/nbm/options/formatter/KotlinFormattingPreviewPane.kt`

Read-only live preview of the Kotlin code sample (`preview.kt` resource). Calls
`collectSettings(tempPrefs)` → `KotlinCodeStylePreferences.loadIntoGlobal(tempPrefs)` →
`KotlinFormatterUtils.formatCode(rawCode, "preview.kt", project, "\n")`.

Debounces updates with a 300 ms Swing Timer. Falls back to showing raw code when no Kotlin project
is open (formatting requires a live PSI factory). Uses `MapPreferences` (in-memory
`AbstractPreferences` backed by `ConcurrentHashMap`) for the temporary settings node.

### KotlinOptionsPanel restructured

`Nbm/src/main/kotlin/io/github/nbplugins/kotlin/nbm/options/KotlinOptionsPanel.kt`

Layout:
- NORTH: `KotlinStyleBar`
- CENTER: `JSplitPane(HORIZONTAL_SPLIT)` — left = `JTabbedPane` (3 formatter tabs), right = `KotlinFormattingPreviewPane`
- `resizeWeight = 0.55`; divider set to 55% in `addNotify()`

`load()` reads `CODE_STYLE_DEFAULTS` from prefs to sync the StyleBar combo, then loads each panel
and schedules a preview refresh. `onStyleApplied()` saves the applied `CodeStyleSettings` to prefs
immediately, reloads all panels, and fires `onChange`.

### Tests

All 415 tests run; 17 failures are pre-existing `HighlightingTest` regressions (NullPointerException
on file parameter — present before E5 changes). New tests:
...
- `KotlinFormattingPreviewPaneTest` — 3 tests (raw code loaded, refresh with no project shows raw code, collectSettings not called without project)

---

## Agreed UI Architecture (implemented)
...
### Step 2 — Remaining 3 formatter panels
...
### Step 3 — Per-project ProjectCustomizer
...
| `Nbm/src/main/kotlin/io/github/nbplugins/kotlin/nbm/options/KotlinOptionsPanel.kt` | Root panel (StyleBar + JSplitPane + preview) |
| `Nbm/src/main/kotlin/io/github/nbplugins/kotlin/nbm/options/formatter/KotlinStyleBar.kt` | Style-preset dropdown (3 built-in presets) |
| `Nbm/src/main/kotlin/io/github/nbplugins/kotlin/nbm/options/formatter/KotlinFormattingPreviewPane.kt` | Live preview pane (300ms debounce, MapPreferences) |
