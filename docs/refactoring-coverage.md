...
| `introduce-type-alias` | Introduce Type Alias | `Ctrl+Alt+Shift+A` | Partial | Supports same-file generic alias extraction with per-occurrence substitutions, including nested and qualified generic user types. Star projections, aliases requiring local type parameters, class-body targets, expressions, and project-wide duplicate search remain unavailable. | F3 |
