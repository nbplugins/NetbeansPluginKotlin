...
- [ ] **A4.3** — `kotlin-compiler`: Maven Central only (no `lib/`); `kotlin-formatter`: compile from embedded sources
...
**Phase 4.3 — Compile from sources (before K2)**

- [ ] `kotlin-compiler`: remove `lib/kotlin-compiler-1.3.72-PATCHED.pom` (already done); original JAR from Maven Central only — no `lib/` copy
- [ ] `kotlin-formatter`: compile from embedded sources (6 `.kt` files in `src/` inside the JAR)
